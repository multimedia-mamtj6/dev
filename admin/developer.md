...
**Verified 2026-07-22 against the reference repo's real code and live output (locally cloned at `mamtj6/infaq/infaq-1`), not just its own docs — which turned out to be stale on this exact point:** its `DATA_STRUCTURE.md` describes `bulan`/`Bulan` fields as full-name-plus-year ("November 2024"), but its actual live `monthly.json`/`perbelanjaan.json` use a short, uppercase, no-year form ("JUN") — the real files are the source of truth, and this repo's `bulan`/`Bulan` fields were fixed to match them exactly (`BULAN_SHORT` in `api/publish-infaq.js`, also fixed there: `"Mar"` was wrong Malay for March, corrected to `"Mac"`, uppercased to match `graf.labels`). `ringkasan.kutipan.tahunIni`/`tahunLepas.tahun` in the `monthly` target were also fixed from `String(year)` to a plain number, matching the reference (`graf.<year>.tahun` was already correct as a string in both — don't "fix" that one). The admin CMS's own `infaq_activity_log` labels (shown in `userlog.html`) deliberately kept the nicer full "Month Year" form via a separate `activityMonthLabel` variable, decoupled from the externally-published `bulan`/`Bulan` fields, so this compatibility fix didn't regress the admin's own log readability. **Still open, not part of this fix:** the reference site's main homepage (`script.js`'s `loadDashboard()`) actually depends on a `data.json` file (`projek` + top-level `tarikhKemaskini`) that this repo's pipeline doesn't produce at all — publishing to it, if this system is ever meant to actually replace the reference site's data source, needs a 4th `TARGETS` entry. `daily.json` (which this repo does produce) currently has zero live consumers in the reference repo — per its own `CHANGELOG.md`, it's intentionally a forward-looking feed for a not-yet-built "future daily tracker page," not a compatibility gap.

**`perbelanjaan` target's `graf` publishes every year present in `infaq_perbelanjaan_bulanan`, not just the current year (fixed 2026-07-23):** the `monthly`/kutipan target always built `graf` for both `year` and `year - 1` (an explicit two-key object literal), but the `perbelanjaan` branch only ever built `graf[year]` — even though `ringkasan.perbelanjaan.tahunLepas.jumlah` on the very same response proves the previous year's rows exist in the table. Found by comparing live `perbelanjaan.json` output against the reference repo, which showed a real (not code) explanation for a missing past-years dropdown — but here it turned out to be a code gap, not missing Sheet data. Fixed by collecting the distinct years present in `perbelanjaanRows` into a `Set` (always union'd with `year` and `year - 1` as a floor, so `paparanBulanIni`/`paparanBulanLepas`'s `JumlahKumulatif` lookups — including the January-edge-case branch above — always have a series to read from even in an empty table) and building one `buildYearlyGraf` entry per year. Matches how the reference frontend's year-selector already works (`script.js`'s `renderPastYearsExpenseCharts` does `Object.keys(graf)` and shows whatever's there — it was never hardcoded to 2 years, only this repo's publish endpoint was). The `monthly`/kutipan target's `graf` is unchanged (still hardcoded to `year`/`year - 1` only) — not touched by this fix, since it wasn't reported as a gap.
