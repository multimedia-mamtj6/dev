...
_Status: **v1 BUILT** (2026-07-18, see "v1 as-built notes"). **v2 BUILT** (2026-07-18, both phases — proxy `api/weather-warning.js` + SEKSYEN parser, see "v2 as-built notes" at the bottom). **v3 BUILT** (2026-07-19, severity-tier colouring). **v4 BUILT** (2026-07-19 — signage page `csr/weather/paparan/` + shared `weather-core.js` extraction, see v4 as-built notes at the bottom). **v5 BUILT** (2026-08-10 — full visual redesign of `paparan/`: dark/light theme, fixed 1920×1080 scaled canvas, MET colour palette, per-tier warning cards, map legend + district labels, see v5 as-built notes at the bottom). **v6 BUILT** (2026-08-18 — `paparan/`-only: marine-bulletin exclusion hardened, new thunderstorm Watch tier, defensive unconfirmed "Continuous Rain" catch on the data.gov.my source, see v6 as-built notes at the bottom). **v8 BUILT** (2026-08-26 — nationwide mode's per-state pill replaced with zoom-gated district labels, `index.html` only, see v8 as-built notes at the bottom). **⚠️ v2's `MET_TOKEN` is currently unavailable (Cloudflare 502 confirmed 2026-08-18) — set/fix the env var in Vercel Dashboard → Project → Settings → Environment Variables, then redeploy. Rain warnings degrade gracefully without it (keeps last-known state), this is not an emergency.** Sibling of `telegram-warning-plan.md` — same data sources, same matching rules; this covers the WEB PAGE._
...
## v5 — visual redesign of `paparan/` (2026-08-10 — BUILT, same session as the plan)

Pure visual-layer redesign of `paparan/index.html` + `paparan/style.css`, requested via a written brief plus two mockup screenshots (light + dark). `weather-core.js` untouched — data/state logic identical to v4, only markup/CSS/purely-presentational JS changed. Full plan at the time: `C:\Users\PC CUSTOM\.claude\plans\redesign-brief-cuaca-pahang-refactored-thacker.md` (outside the repo, session-local).

- **Fixed 1920×1080 canvas** (`#stage`), uniformly scaled via JS (`Math.min(vw/1920, vh/1080)`, `transform: scale()`) to fit any real screen — replaces the old `clamp()`-based vw-fluid sizing entirely (fixed-px tuned for the canvas now, since fluid sizing fights a uniform outer scale).
- **Dark-default theme** with light alt, `localStorage['theme']`, `<html data-theme>` set pre-paint by a new `<head>` anti-flash script (paparan had none before) — same convention as `calendar/hijri/tarikh-penting/`'s Tailwind toggle, hand-adapted to CSS custom properties since this page loads no Tailwind. Toggle is a Malay text-label pill ("Mod Gelap"/"Mod Terang" — whichever mode a click switches *to*), not an icon swap, per the mockups. `body.map-dark` (separate from `data-theme`) drives a CSS filter on `.leaflet-tile-pane` for dark map tiles.
- **Archivo (headings) + Inter (body)** via Google Fonts, replacing the system-font stack.
- **Severity colours now match MET's own palette** (carried over from the earlier same-session colour-matching work on `weather-core.js`): amaran/Ribut Petir blue `#2516d2`, waspada yellow `#fdfe00`, buruk orange `#fea402`, bahaya red `#fa0101`. Unwarned-district border changed to grey `#8a8a8a` via a **paparan-local** `PAPARAN_DEFAULT_STYLE` override (`{...defaultStyle, color:'#8a8a8a'}`) — `weather-core.js`'s shared `defaultStyle` was deliberately left untouched since the interactive page also reads it.
- **District name-pill labels** on the map — new, via Leaflet `bindTooltip(name, {permanent:true, className:'district-label-pill'})` in `loadGeoJSON()`'s `onEachFeature`; didn't exist before this version.
- **Floating "Status Daerah" legend**, top-right of the map panel — static 4-row HTML, no JS.
- **Warning cards restructured**: one `.warn-card-tier-row` per active severity tier (was one flat pill list per card regardless of tier mix), each row showing its own swatch + label + "sehingga" time. Confirmed via direct read of `weather-core.js` that MET bulletins carry exactly ONE `valid_to` per bulletin (no per-section expiry) — every tier row within a card intentionally reuses that same timestamp, documented in a code comment so it isn't "fixed" into a fabricated per-tier value later. The single always-`amaran`-tier row in the Ribut Petir card is labeled "Ribut Petir / Hujan Lebat" (not the generic `TIER_LABEL['amaran']` = "Amaran") to match the card title/legend.
- **Bahaya-tier cards** get a `tier-bahaya-active` class → animated pulsing glow (`box-shadow` keyframe, ~3s loop) plus a small pulsing dot next to the card title.
- **Countdown display resolved by user choice mid-build**: the mockups show absolute time only ("sehingga 12:40 PTG"), but the written brief explicitly required preserving the live "X minit lagi/lepas" behaviour — landed on absolute time + a small muted relative chip together (e.g. "sehingga 12:40 PTG · 59 minit lagi"), keeping `relPast`/`relFuture` visibly working via the existing 60s tick.
- New `#live-clock`/`#live-date` header block, folded into the existing `TICK_MS` (60s) interval — no new interval added.
- Every id the existing JS read/wrote (`#map`, `#forecast-temp`, `#slot-*`, `#card-thunder`, `#card-rain`, `#test-mode-banner`) survived unchanged; `#page-footer` split into `#footer-source` (static) + `#footer-updated` (JS-written) children.
- Visual verification: headless-Chrome screenshots via local `python -m http.server` — default dark render (`district:Kuantan,Pekan|rain:buruk:Maran;waspada:Temerloh,Bera`, the `|`-combinator added earlier this session), the `rain:bahaya:Maran` pulsing-glow state, and `?debugMap=1` at a smaller window size (confirmed the scale-to-fit + centered letterboxing behaves correctly at non-1920×1080 sizes, and the debug drag/zoom readout still renders). Light theme and animation motion itself weren't screenshot-verified (static screenshots can't show CSS animation, and toggling theme requires a live click) — spot-check both by hand before calling this done end-to-end.
...
## v6 — thunderstorm parsing improvements + defensive rain catch, `paparan/` only (2026-08-18 — BUILT)

`MET_TOKEN`/`api/weather-warning.js` is currently returning a Cloudflare 502 (confirmed live, unrelated to this change — token access is unavailable for now). A teammate who built a similar weather feature described getting *both* thunderstorm and continuous-rain data from the same free `api.data.gov.my/weather/warning` endpoint via heading-text keyword matching, plus a Warning-vs-Watch severity split. Checked directly before acting on it: a live unfiltered pull from that endpoint and the official docs at `developer.data.gov.my/realtime-api/weather` show no "Continuous Rain" category there today — matches the existing "live gap verified 2026-07-18" comment in `api/weather-warning.js`. Not conclusive (Pahang rain warnings are rare, so absence in one snapshot isn't proof), so the response is a **defensive catch**, not a source replacement — MET_TOKEN/metapi2 stays the real primary rain source, per the standing fallback philosophy in this doc ("drop rain warnings from the page rather than ship the token").

Two more things the same live check surfaced and got fixed:

- **`extractPahangDistricts()`'s marine check now runs BEFORE the district-parenthetical match** (was after) — a genuine land bulletin never says "waters of"/"perairan" (verified: land bulletins say "over the **states** of ... Pahang (...)"), so this only changes behavior for a sea bulletin that happens to embed a `Pahang (...)`-shaped substring, which previously would have been misread as a land-district hit.
- **`isMarineBulletin()` now also matches the heading directly** (`/CATEGORY WARNING ON STRONG WINDS/i`) — a live "FIRST CATEGORY WARNING ON STRONG WINDS AND ROUGH SEAS" bulletin was observed 2026-08-18 (not Pahang-relevant that day, found via a broad unfiltered sample); its whole multi-section text is sea-themed throughout, so checking the heading is a more direct signal than body-text phrasing alone.
- **New `watch` tier** (`TIER_RANK`/`TIER_COLORS`/`TIER_LABEL`, light blue `#93c5fd`) — `thunderstormTier()` reads `"Warning on Thunderstorms"` (unconfirmed/early) vs `"Thunderstorms Warning"` (confirmed, existing `amaran`) from the heading. Flows through the existing tier-generic map/card/legend code with no changes needed there beyond adding the 5th legend row and a `tierRowLabel()` case ("Ribut Petir (Peringatan)").
- **Defensive `isContinuousRainHeading()`/`extractRainFromThunderSource()`/`reclassifyRainWarnings()`** in `weather-core.js`, wired into `fetchActiveWarnings()` — reclassifies any data.gov.my item whose heading matches "Continuous Rain"/"Hujan Berterusan" into the same `{source:'rain', pahang}` shape `normalizeRainWarnings()` produces, so it renders in the "Amaran Hujan Berterusan" card instead of being mislabeled as a thunderstorm. **Not verifiable against a real bulletin** — no live example has ever been observed on this endpoint, so this is best-effort parsing of data.gov.my's flat text shape (deliberately does NOT assume metapi2's `SECTION A:` prose structure, which is a different API). Revisit if a real instance is ever captured.
- New `?testWarning=` fixtures: `watch` / `watch:<districts>` (exercises the new tier), `thunder-rain` / `thunder-rain:<tier>:<districts>` (built as a flat data.gov.my-shaped payload, run through the real `reclassifyRainWarnings()` rather than a hand-built mimic, since there's no live example to test against otherwise).
- Scope: `paparan/weather-core.js` + `paparan/index.html` (+ its `style.css` for the new `--tier-watch` token) only. `csr/weather/weather-core.js` (root interactive page) has the identical marine/district-ordering and untiered-thunderstorm logic and would benefit from the same fixes — offered as a fast-follow, not bundled into this change (same pattern as this session's earlier `?testWarning=` case-insensitivity fix).
- Verified: Playwright against local `python -m http.server` — all new fixtures (`watch`, `watch:Temerloh,Maran`, `thunder-rain`, `thunder-rain:buruk:Maran`) render in the correct card with correct tier/districts; existing fixtures (`district:`, `marine`, `rain:`, `none`) regression-checked unchanged; a synthetic marine-bulletin-with-`Pahang(...)`-shaped-text case confirmed the reordering excludes it correctly while a genuine land bulletin is unaffected; visual screenshot confirms the light-blue watch tier paints correctly on map, legend, card header badge, and pills (legible dark text on the light background).

## v7 — state-boundary layer switched from derived (Turf.js) to sourced (`malaysia.state.geojson`), `csr/weather/index.html` only (2026-08-24 — BUILT)

The nationwide view's black/white state-boundary outline (added in an earlier session, on top of the district polygons from `malaysia.district.geojson`) was previously **computed at runtime**: pad every district out 3km via `turf.buffer`, `turf.dissolve()` by `properties.state`, shrink back 3km — needed because adjacent districts' shared edges in the source file aren't precisely coincident, so a plain dissolve left the seams unmerged. Reported live: this buffer round-trip isn't lossless — it smooths away anything narrower than the buffer radius, visible as a doubled/jagged line on tightly winding river borders (e.g. Terengganu/Perak).

Fix: the same `mptwaktusolat/jakim.geojson` repo (checked via its actual file listing, not assumed from the README) ships a dedicated, pre-made **state-level** file, `malaysia.state.geojson` — 16 features, one per state, `properties.state` using the identical 3-letter codes already used throughout this codebase. Verified its per-state bounding boxes match the district file's to within ~10m (same source survey), so it registers correctly against the existing district polygons with no coastline mismatch. `buildStateBoundaryLayer()` now just fetches and renders that file directly (`fetchWithTimeout`, same graceful-degradation posture as every other fetch in this file) — no client-side geometry computation, no smoothing artifact. The Turf.js `<script>` tag is gone entirely — it had no other use in this file.

(Separately inspected `peta.waktusolat.app`'s own JS bundle, the reference the user pointed at: it's a MapLibre GL app that renders `malaysia.district-jakim.geojson` directly with a per-district `fill-color` keyed to `properties.state` plus a uniform white district-line layer — no dissolve step on their side at all, the "clean state look" is an optical effect of same-state districts sharing one fill color under WebGL vector rendering. Confirms the district data itself was never the problem; doesn't transfer directly here since this page's fills are reserved for warning-tier severity, not neutral per-state grouping.)

- Verified offline: all 16 `STATE_CONFIG` state codes have a matching feature in `malaysia.state.geojson` (no naming drift between the two files).
- Verified live via Playwright: nationwide view renders the new boundary layer correctly in both themes (color still flips on toggle via the existing `stateBoundaryColor()`/`initThemeToggle()` codepath, unchanged); zoomed into the exact previously-broken Terengganu/Perak seam and confirmed a single clean line in both light and dark screenshots, no doubled/jagged artifact.

## v8 — zoom-gated district labels in nationwide mode, `csr/weather/index.html` only (2026-08-26 — BUILT)

Replaced the always-on per-state pill (`addStateLabels()`, one permanent tooltip per state centroid) with a single zoom-gated tooltip mode per district layer:

- **Below `NATIONWIDE_LABEL_ZOOM_THRESHOLD`** (hand-tuned via `?debugzoom=1` to `8.63`): no permanent text at all. Hovering (desktop) or tapping (touch) a district shows its **state name only**, on demand — bound via `bindNationwideHoverTooltip()`. Touch has no `mouseout` to close a hover tooltip, so a tap explicitly opens it and a 2.5s timer closes it again (`IS_TOUCH_PRIMARY`, `window.matchMedia('(pointer: coarse)')`); desktop just gets Leaflet's own default hover show/hide.
- **At/above the threshold**: every district's tooltip switches to automatic + permanent, reading `"District, State"` (e.g. `"Kuantan, Pahang"`) — no interaction needed, matches showing many district names at once when zoomed into a regional view.
- `applyNationwideLabelMode()` re-evaluates all of `geojsonLayer`'s tooltips against the current zoom on `zoomend` (`unbindTooltip()` + rebind in the correct mode) — a no-op when the mode hasn't changed, so it's cheap to call on every zoom tick. Also called once right after `resetMapView()` in `loadGeoJSON()`, covering the edge case where the initial `fitBounds()` view already sits above the threshold.
- **`?debugzoom=1`** — on-screen current-zoom readout (`initDebugZoomReadout()`), same "let them find it, then hardcode it" technique as `paparan/`'s `?debugMap=1` framing tool, used to hand-tune the threshold live rather than guessing it.
- **`NATIONWIDE_DEFAULT_ZOOM` (`6.53`)**, also hand-tuned via `?debugzoom=1` — nationwide mode's own default framing zoom, replacing a bare `fitBounds()` (which read as either too tight or too loose). `resetMapView()` keeps `fitBounds()`'s own computed center (`geojsonLayer.getBounds().getCenter()`) but sets the view in one atomic `setView()` call rather than `fitBounds()` immediately followed by `setZoom()` — the two-call version raced `fitBounds()`'s own zoom animation and the `setZoom()` got silently dropped (confirmed live: default zoom stayed at the raw `fitBounds()` value, ~6.65, until switched to the atomic form).
- `.state-label-pill` CSS class removed; nationwide's on-demand state tooltip reuses the same pill look under a new class, `.state-hover-pill` (kept in `style.css`'s tooltip-arrow-hiding rule alongside `.district-label-pill`).
- Scope: nationwide (`"malaysia"`) mode only. Per-state view (e.g. `?state=pahang`) keeps its existing always-on, non-zoom-gated district-only labels unchanged. `paparan/` (fixed-zoom, non-interactive, Pahang-only) untouched.
