...
**Also writes an `activity_log` row on success:** after the GitHub push succeeds, `publish.js` looks up the acting admin's `name` from the `admins` table (via the `actorEmail` already extracted from the `/auth/v1/user` check) and POSTs an `activity_log` row directly via the Supabase REST API using the service-role key — a second, small Supabase round-trip, deliberately kept cheap since Terbitkan is a rare action. Wrapped in try/catch so a logging failure can never turn a successful publish into an error response to the admin.
