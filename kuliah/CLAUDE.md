...
## Month navigation & the two date-overflow bugs (fixed 2026-09-01)

`index.html?bulan=depan` shows next month's table (button in the page header). **Two related bugs, both stemming from JS `Date`'s day-overflow behavior, were found and fixed in the same session — read both before touching `script.js`'s date math again:**

1. **`?bulan=depan` showed the *wrong* next month whenever today's day-of-month exceeds the actual next month's day count** (e.g. viewed on Aug 29-31 → showed October, not September). Root cause: `baseDate.setMonth(baseDate.getMonth() + 1)` was called on a `Date` still holding today's day-of-month (31). September only has 30 days, so `Date` silently rolls the overflow into October 1 instead of clamping — no error, just a wrong month, and only on the 3-4 days per month where the day count actually overflows (which is why it went unnoticed for a long time). **Fixed by calling `baseDate.setDate(1)` before `setMonth(+1)`** ([script.js:580-584](kuliah/jadual/script.js#L580-L584)) — pin to the 1st first, so there's nothing left to overflow.
2. **The mobile "Hari Esok" dropdown option, on the last day of the month, always showed "Data jadual untuk esok belum tersedia." even when next month's schedule was already published.** This one is *not* the same overflow bug — `tomorrowDate.setDate(+1)` (used for the Hari Esok date itself) handles month rollover correctly, since `setDate` clamps/rolls by design and there's no fixed-day interaction. The actual bug: `renderTodayCard()`'s day lookup only ever searched the *currently-loaded* month's `senaraiHari` array (`cachedSenaraiHari`, populated once at boot for the current `monthKey` only) — tomorrow's date, when it belongs to next month, was never in that array, so the lookup always came back empty regardless of whether next month was actually published in `jsonData.months`. **Fixed by caching the full `jsonData.months` object (`cachedMonthsData`, [script.js:12](kuliah/jadual/script.js#L12)) and falling back to `cachedMonthsData[targetMonthKey]` when the target date isn't found in the currently-loaded month** ([script.js:422-428](kuliah/jadual/script.js#L422-L428)).

**General lesson for this file:** any `Date` math that combines `setMonth()`/`setFullYear()` with a day-of-month that might not exist in the target month needs an explicit `setDate(1)` (or equivalent clamp) first — `Date` overflows silently, never throws, and the bug only reproduces on specific calendar days, so it will not show up in ad-hoc testing done on an arbitrary day. Separately: any per-day data lookup that's scoped to "the currently loaded month" needs to be checked for whether the date it's given can ever fall outside that month (today/tomorrow pairs are the recurring case here) — the fix in both cases was found by reading the exact `Date` calls line-by-line, not by guessing from symptoms.
...
`createLectureBlock()`/`createMobileLectureBlock()` (`kuliah/jadual/script.js`) relabel the session from `Subuh`/`Maghrib` to `Kuliah Subuh Khas`/`Kuliah Maghrib Khas` whenever `khas` is true, add an `is-khas` class (desktop: on `.lecture-block`; mobile: on `.lecture-block-v2` directly, since there's no shared cell wrapper there), and — for a Khas day that's also pending — simplify the sub-label to just "Akan Diumumkan" (the "Khas" meaning now lives in the main label). **This also fixed a pre-existing mislabeling:** the pending sub-label used to unconditionally say "Ceramah Khas — Akan Diumumkan" for *any* pending session regardless of whether it was actually a Khas one — now a plain (non-Khas) pending session just shows generic "Akan Diumumkan." (`kuliah/paparan/`'s own separate pending message was deliberately left untouched — see below, out of scope.)

**Desktop whole-cell purple/green highlight is computed in JS, not CSS `:has()` (fixed 2026-07-29):** the original build colored the day cell via `.lecture-content:has(.lecture-block.is-khas)` (Khas, purple) and `.lecture-content:has(.yasin-block):not(:has(.lecture-block + .lecture-block))` (solo Yasin, green) — both worked in any modern desktop browser but were found to silently not render at all on the actual Xibo signage player (screenshot comparison: "Kuliah Hari Ini" yellow and other plain-class highlights showed fine, only the two `:has()`-based ones were missing). Root cause: `:has()` only shipped in Chromium 105 (mid-2022), and Xibo's Windows Player embeds Microsoft Edge WebView2 — normally auto-updating, but signage boxes are frequently offline/locked-down, so the Runtime can be stuck on an old build indefinitely with no error, just a silently-unmatched selector. **Fixed by moving the "does this cell contain a Khas/Yasin block, and is it sharing the cell with another session" decision into `buildDayCell()` (`script.js`)**, which now scans the already-built `blocks` HTML string (`blocks.match(/class="lecture-block\b/g)` for the shared-vs-solo count, `/\bis-khas\b/`/`/\byasin-block\b/` regexes for presence) and adds `.has-khas` / `.has-yasin-solo` / `.has-yasin-shared` directly onto the `.lecture-content` wrapper div. `style.css` was updated to match on these plain classes instead of `:has()` — same visual result, works on any renderer. **If you ever see `:has(` reintroduced into `kuliah/jadual/style.css`, treat it as a regression of this exact bug** — verified against real production July 2026 data (solo Yasin on the 9th/23rd, shared Yasin found in August's data, Khas on the 27th) via a `node -e` script loading `script.js` through `vm` and calling `buildDayCell()` directly, same harness style as `news/developer.md`'s testing convention.
