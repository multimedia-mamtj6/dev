...
- **`default.svg`'s Arabic line uses a self-embedded, subsetted web font — not a system/Google-Fonts font-family, deliberately (fixed 2026-07-29):** originally `font-family="Georgia, 'Times New Roman', serif"`, neither of which has real Arabic shaping — the browser silently falls back to whatever font the OS substitutes for the Arabic Unicode range, and that substitution isn't guaranteed consistent across machines. On the Xibo player's WebView2 environment this landed on a font/fallback path lacking proper Arabic contextual shaping, rendering each letter in its isolated form instead of joined cursive script, while looking correct on a dev machine with a different fallback. **This file is loaded via a plain `<img src>`** (`script.js`'s `getDefaultImage()`/image-slide builder), and an SVG used as an image is sandboxed — it cannot fetch an external stylesheet or `@font-face` URL, so adding a Google Fonts `<link>` to `index.html` would have had zero effect. Fixed by embedding a base64 WOFF2 `@font-face` (Scheherazade New Bold, subsetted via Google Fonts' `text=` param to just the ~10 characters this slide uses, ~7.7KB) directly inside a `<style><![CDATA[...]]></style>` block in `default.svg` itself — CDATA is required, not optional, since raw `<`/`>` in an unwrapped `<style>` breaks XML parsing (SVG is strict XML, not HTML). **If any future slide/asset in this folder needs non-Latin text rendered via `<img>`-loaded SVG (or any other sandboxed-image context), embed the font the same way — never rely on system font-fallback for a script that needs real shaping (Arabic, and similarly any complex script).**
