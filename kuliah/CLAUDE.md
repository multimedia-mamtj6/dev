...
**Poster tap-to-enlarge lightbox (added 2026-07-26):** tapping any `.poster-img` in the today-card opens a full-screen dark-backdrop lightbox (`#poster-lightbox`, static hidden markup in `index.html`, same `[hidden]`-toggled convention as `#khas-legend`). `initPosterLightbox()` (`script.js`) wires this with a single **delegated** `click` listener on `document` for the open trigger — attached once at boot, not per-image — specifically because `renderTodayCard()` replaces the today-card's inner HTML on every day-select change; a per-element listener would need re-attaching on every render (same reasoning as the Hijri writer's re-query pattern above). Tapping the backdrop or the enlarged image itself closes it (both are clicks inside `#poster-lightbox`), plus a visible × button; `body.no-scroll` locks background scroll while open. Scoped to the today-card only — no poster image renders anywhere else (desktop table, monthly card list, `kuliah/paparan/`).
